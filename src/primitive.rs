...
pub type Compiler = dyn FnMut(Value, BTreeMap<String, RefPrimitive>) -> Primitive;
                    ...
                    match p {
                        ...
                        _ => {
                            ...
                            parameters_formatted
                                .push(char::from_digit(idx as u32, 10).unwrap_or('0'));
                        }
                    }
                    if idx < len {
...
impl Add for Primitive {
    fn add(&self, rhs: &Self) -> Self {
        match (self.clone(), rhs.clone()) {
            ...
            (l, r) => Primitive::Error(format!("illegal call to add() => left: {l} right: {r}")),
        }
    }
}
...
impl Rem for Primitive {
    fn rem(&self, rhs: &Self) -> Self {
        match (self.clone(), rhs.clone()) {
            ...
            (l, r) => Primitive::Error(format!("illegal call to rem() => left: {l} right: {r}")),
        }
    }
}
...
impl ToBool for Primitive {
    fn to_bool(&self) -> Self {
        match self {
            Primitive::Ref(s) => {
                let lock = s.read().expect("TO_BOOL ERORR: could not acquire lock!");
                lock.to_bool()
            }
            ...
            Primitive::String(s) => match s.parse::<bool>() {
                ...
                Err(e) => Primitive::Error(format!("invalid cast to bool: {self}, {e}")),
            },
...
impl ToNumber for Primitive {
    fn to_int(&self) -> Self {
        match self {
            Primitive::Ref(s) => {
                let lock = s.read().expect("TO_INT ERORR: could not acquire lock!");
                lock.to_int()
            }
            ...
            Primitive::String(s) => match s.parse::<i128>() {
                ...
                Err(e) => Primitive::Error(format!("invalid cast to int: {self}, {e}")),
            },
    ...
    fn to_double(&self) -> Self {
        match self {
            ...
            Primitive::String(s) => match s.parse::<f64>() {
                ...
                Err(e) => Primitive::Error(format!("invalid cast to double: {self}, {e}")),
            },
    ...
    fn partial_cmp(&self, other: &Self) -> Option<Ordering> {
        match (self, other) {
            ...
            (
                ...
                if pl.eq(pr) && el.iter().zip(er.iter()).filter(|&(a, b)| a != b).count() == 0 {
                    Some(Ordering::Equal)
                } else {
                    None
                }
            ...
            (Primitive::EarlyReturn(l), Primitive::EarlyReturn(r)) => l.partial_cmp(r),
...
impl Array for Primitive {
    fn index_at(&self, rhs: &Primitive) -> Primitive {
        match (self, rhs) {
            (Primitive::Ref(l), Primitive::Ref(r)) => {
                let l = l.read().expect("INDEX_AT L ERORR: could not acquire lock!");

                let r = r.read().expect("INDEX_AT R ERORR: could not acquire lock!");
                l.index_at(&r)
            }
            (Primitive::Ref(l), r) => {
                let l = l.read().expect("INDEX_AT L ERORR: could not acquire lock!");
    ...
    fn len(&self) -> Primitive {
        match self {
            ...
            _ => Primitive::Error(format!("call to len() on a non array value => {self}")),
        }
    }
    ...
    fn swap_mem(&mut self, rhs: &mut Primitive, index: &Primitive) -> Primitive {
        match (self, index) {
            ...
            (Primitive::Ref(l), Primitive::Ref(r)) => {
                ...
                let r = r.read().expect("SWAP_MEM R ERORR: could not acquire lock!");
                l.swap_mem(rhs, &r)
            }
            (Primitive::Ref(l), _) => {
                let mut l = l
    ...
    fn remove(&mut self, key: &Primitive) -> anyhow::Result<()> {
        match (self, key) {
            ...
            // using Arc::ptr_eq(&arc1, &arc2) might be safer
            (Primitive::Ref(l), Primitive::Ref(r)) => {
                let mut l = l.write().expect("REMOVE L ERORR: could not acquire lock!");

                let r = r.read().expect("REMOVE R ERORR: could not acquire lock!");
                l.remove(&r)
            }
            (Primitive::Ref(l), _) => {
                let mut l = l.write().expect("REMOVE L ERORR: could not acquire lock!");
